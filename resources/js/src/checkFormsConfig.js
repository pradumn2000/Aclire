...
export const CHECK_FORM_CONFIG = {
  education: {
    ...
    fields: [
    { key: "scope", label: "National / International", type: "radio", options: ["National", "International"], required: true },
    { key: "amount", label: "Amount (₹)", type: "amount", required: false },   // ← new: admin-editable, client read-only, prefilled from case check_rates
    { key: "qualification",  label: "Education Qualification", type: "select", options: QUALIFICATION_OPTIONS, required: true },
    { key: "instituteName",  label: "Institute Name",  type: "select_dynamic", source: "institutions",
      filterBy: "scope", extraParams: { type: "university" }, required: true },      // ← added filterBy + extraParams
    { key: "universityName", label: "University Name", type: "select_dynamic", source: "institutions",
...
export function computeCheckStatus(checkKey, data) {
  ...
  // "amount" is metadata (billing), not a data-completeness field — never
  // let it block/count toward a check's submitted/pending status.
  const requiredFields = config.fields.filter(f => f.required && f.key !== "amount").map(f => f.key);
  const fieldsComplete  = requiredFields.every(k => fields[k] && String(fields[k]).trim() !== "");
  const docsComplete    = config.documents.length === 0 || config.documents.every(d => docs[d.key]);
