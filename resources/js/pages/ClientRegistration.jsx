...
// //       const agreedChecks = has
// //         ? p.agreedChecks.filter((c) => c !== key)
// //         : [...p.agreedChecks, key];
// //       const checkRates = { ...p.checkRates };
// //       if (has) delete checkRates[key];
// //       else checkRates[key] = DEFAULT_RATES[key] ?? 0;
// //       return { ...p, agreedChecks, checkRates };
// //     });

// //   const setRate = (key, value) =>
// //     setForm((p) => ({ ...p, checkRates: { ...p.checkRates, [key]: Number(value) || 0 } }));
...
// //                 </div>
// //               ))}
// //               {form.agreedChecks.length > 0 && (
// //                 <div style={{ padding: "10px 0 0" }}>
// //                   <span style={{ color: "#94a3b8", fontSize: "13px" }}>Rate Card</span>
// //                   <div style={{ marginTop: "8px", display: "flex", flexDirection: "column", gap: "4px" }}>
// //                     {form.agreedChecks.map((c) => (
// //                       <div key={c} style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>
// //                         <span style={{ color: "#cad2e1" }}>{CHECK_OPTIONS.find(o => o.key === c)?.label}</span>
// //                         <span style={{ color: "#02c39a", fontWeight: 700 }}>₹{form.checkRates[c] ?? 0}</span>
// //                       </div>
// //                     ))}
// //                   </div>
// //                 </div>
...
// //                     {/* Rate card — shown only for selected checks */}
// //                     {form.agreedChecks.length > 0 && (
// //                       <div>
// //                         <label style={labelStyle}>
// //                           Rate Card
// //                           <span style={{ color: "#94a3b8", fontWeight: 400, marginLeft: "8px" }}>(₹ per check)</span>
// //                         </label>
// //                         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginTop: "10px" }}>
// //                           {form.agreedChecks.map((key) => {
// //                             const opt = CHECK_OPTIONS.find((c) => c.key === key);
// //                             return (
// //                               <div key={key} style={{
// //                                 display: "flex", alignItems: "center", justifyContent: "space-between",
// //                                 gap: "8px", padding: "10px 14px", borderRadius: "10px",
// //                                 border: "2px solid #3a4faa", background: "rgba(255,255,255,0.04)",
// //                               }}>
// //                                 <span style={{ color: "#cad2e1", fontSize: "13px", fontWeight: 600 }}>
// //                                   {opt?.label}
// //                                 </span>
...
//           billingMode:    form.billingMode,
//           agreedChecks:   form.agreedChecks,
//           checkRates:     {}, // rate card removed from registration
//         }),
//       });
...
//               {/* ── STEP 2: Billing + Check Types (Rate Card removed) ─── */}
//               {step === 2 && (
//                 <>
//                   <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
...
//                     {/* Check types — Rate Card input removed */}
//                     <div>
//                       <label style={labelStyle}>
//                         Agreed Check Types *
//                         <span style={{ color: "#94a3b8", fontWeight: 400, marginLeft: "8px" }}>(select all that apply)</span>
...
//   textTransform: "uppercase", letterSpacing: "0.05em",
// };
// ClientRegistration.jsx — Registration form for client companies
// Route: /client-register (public)
// On submit: POST /api/clients/register → returns token + user with role="client"
//
// Fixes applied:
//   1. Added required "Address" field (backend requires it — was missing,
//      causing a silent 422).
//   2. Password rule now matches backend ('required|min:8') instead of the
//      old 'digits:6' mismatch that made every real registration fail
//      validation.
//   3. Removed the silent localStorage/registerClient fallback on API
//      failure. Previously, ANY backend error (422, 500, etc.) fell through
//      to a fake local "success" screen with no real user ever created in
//      the DB — which is why login worked from this form but failed later
//      via the real /api/login endpoint. Now real errors are shown to the
