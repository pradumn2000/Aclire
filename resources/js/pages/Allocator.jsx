...
//   const updateCardAssignment = (caseId, checkKey, assigneeKey) => {
//     setAssignments(prev => ({ ...prev, [assignKey(caseId, checkKey)]: assigneeKey }));
//   };
...
//                                               onChange={(e) => updateCardAssignment(c.case_id, col.key, e.target.value)}
...
//                           <div key={row.key} className="cab-progress-item" id={`progress-${row.key}`}
...
//                             </div>
//                             <div className="cab-progress-bar-bg">
//                               <div className="cab-progress-bar-fill" id={`fill-${row.key}`}
...
//           {/* Notification Toast */}
//           <div id="toast" className={`cab-toast ${showToast ? "" : "cab-hidden"}`}>
//             <i className="fa-solid fa-circle-check cab-toast-icon"></i>
//             <span id="toast-message">Assignments saved locally (not yet persisted to backend).</span>
//           </div>
...
export default function Allocator() {
  ...
  const handleConfirm = () => {
    ...
    // For now this only acknowledges the local selections made above.
    setToastMessage(
      `${realAssignments.length} assignment${realAssignments.length > 1 ? "s" : ""} confirmed successfully.`
    );
    setToastError(false);
    setShowToast(true);
    setTimeout(() => setShowToast(false), 2500);
      ...
      <section id="noSidebar">
        ...
        <main>
          ...
          <div
            ...
            style={toastError ? { background: "#fff5f5", borderColor: "#fca5a5", color: "#dc2626" } : undefined}
          >
            <i
