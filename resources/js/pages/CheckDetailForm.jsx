...
export default function CheckDetailForm({ caseObj, checkKey, onClose, onSaved }) {
  ...
  // ── Amount — lives on the case's own check_rates snapshot, NOT inside
  //    `fields`, so it can never travel through the same save path a
  //    client is allowed to hit. Prefilled from whatever was set when the
  //    case was created / last edited by admin in AddCase.jsx.
  const [amount, setAmount] = useState(caseObj?.check_rates?.[checkKey] ?? "");
  ...
  // ── Normalize any date-ish value (including full ISO timestamps from
  //    Laravel, e.g. "2026-08-03T00:00:00.000000Z") down to yyyy-MM-dd so
  //    <input type="date"> never rejects the value with a console warning.
  ...
  const handleSave = async () => {
    ...
    try {
      const body = { fields };
      // Only admins ever send `amount` — a client's request body never
      // includes it, regardless of what the disabled input currently shows.
      if (isAdmin) body.amount = Number(amount) || 0;
      ...
      onSaved?.(checkKey, {
        fields,
        documents,
        check_rates: data.check_rates ?? { ...(caseObj.check_rates || {}), [checkKey]: amount },
      });
      onClose();
    } catch {
      setError("Server error while saving.");
                  ...
                  <div key={f.key} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    ...
                    {f.type === "amount" ? (
                      <div style={{
                        ...
                        background: !isAdmin ? "#f1f5f9" : inputStyle.background,
                      }}>
                        ...
                        <input
                          ...
                          style={{
                            ...
                            width: "100%", fontSize: "13px", color: "#1e293b",
                      ...
                      <input
                        type="date"
                        value={toDateInputValue(fields[f.key])}
                        onChange={(e) => setField(f.key, e.target.value)}
                        style={inputStyle}
                      />
