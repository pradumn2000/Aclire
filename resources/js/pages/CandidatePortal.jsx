...
//   );
// }
// CandidatePortal.jsx — Token-gated portal for candidates
// Route: /candidate/:token
import { useState, useEffect } from "react";
import { useParams } from "react-router-dom";
import { API_URL } from "../src/config";
import { CHECK_FORM_CONFIG } from "../src/checkFormsConfig";

const NORMALIZE_CHECK_KEY = {
  emp: "employment", edu: "education", addr: "address", db: "database",
  drug_test: "drug", courtroom: "court",
};
const normalizeKey = (k) => NORMALIZE_CHECK_KEY[k] || k;

const COMMON_DOCS = [
  { key: "photo",   label: "Passport Size Photo" },
  { key: "govt_id", label: "Government ID (Aadhar/PAN)" },
];
...
export default function CandidatePortal() {
  ...
  const [step, setStep]         = useState("loading"); // loading | form | submitted | expired | invalid
  const [linkInfo, setLinkInfo] = useState(null);       // { candidateName, email, caseId, checkType, checks, expiresAt }
  const [checkDetails, setCheckDetails] = useState({}); // existing case.check_details
  const [fieldsState, setFieldsState]   = useState({}); // { [checkKey]: { [fieldKey]: value } }
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/candidate-link/${token}`)
      .then(async (res) => {
        ...
        if (data.submitted) { setStep("submitted"); return; }
        if (!res.ok) { setStep("invalid"); return; }

        const checks = (data.link.checks || []).map(normalizeKey);
        setLinkInfo({ ...data.link, checks, checkType: data.link.checkType ? normalizeKey(data.link.checkType) : null });
        setCheckDetails(data.checkDetails || {});

        const initFields = {};
        checks.forEach((ck) => { initFields[ck] = (data.checkDetails?.[ck]?.fields) || {}; });
  ...
  const setField = (checkKey, fieldKey, value) => {
    setFieldsState((p) => ({ ...p, [checkKey]: { ...(p[checkKey] || {}), [fieldKey]: value } }));
  };

  const handleUpload = async (checkKey, docKey, file) => {
    if (!file) return;
    setUploadingKey(`${checkKey}:${docKey}`);
    setError("");
    try {
      const formData = new FormData();
      formData.append("check_type", checkKey);
      formData.append("document_key", docKey);
      formData.append("file", file);

      const res = await fetch(`${API_URL}/api/candidate-link/${token}/documents`, {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
      ...
      setCheckDetails((p) => ({
        ...
        [checkKey]: {
          ...(p[checkKey] || {}),
          documents: { ...(p[checkKey]?.documents || {}), [docKey]: { name: file.name, url: data.url } },
        },
      }));
    } catch {
  ...
  const missingRequiredFields = () => {
    const missing = [];
    (linkInfo?.checks || []).forEach((ck) => {
      const config = CHECK_FORM_CONFIG[ck];
      if (!config) return;
      config.fields.filter((f) => f.required).forEach((f) => {
        if (!fieldsState[ck]?.[f.key]?.toString().trim()) {
          missing.push(`${config.label}: ${f.label}`);
        }
      });
    });
    return missing;
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Save fields for every check this link covers
      for (const ck of linkInfo.checks) {
        const config = CHECK_FORM_CONFIG[ck];
        if (!config || config.fields.length === 0) continue;
        await fetch(`${API_URL}/api/candidate-link/${token}/fields`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
          body: JSON.stringify({ check_type: ck, fields: fieldsState[ck] || {} }),
        });
      }
  ...
  if (step === "expired") {
    return (
      <CenteredCard>
        <div style={{ fontSize: "48px", marginBottom: "16px" }}>⏰</div>
        <h2 style={{ color: "#1e2761", marginBottom: "8px" }}>Link Expired</h2>
        <p style={{ color: "#64748b", fontSize: "14px" }}>This link has expired. Please contact your HR team to generate a new one.</p>
      </CenteredCard>
    );
  }
  if (step === "submitted") {
    return (
      <CenteredCard>
        <div style={{ width: "64px", height: "64px", background: "#02c39a", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px", margin: "0 auto 20px" }}>✓</div>
        <h2 style={{ color: "#1e2761", marginBottom: "8px" }}>Submitted!</h2>
        <p style={{ color: "#64748b", fontSize: "14px", maxWidth: "320px", margin: "0 auto", textAlign: "center" }}>
          Thank you{linkInfo?.candidateName ? `, ${linkInfo.candidateName}` : ""}. Your information has been received and the team will review it shortly.
        </p>
      </CenteredCard>
    );
  }

  const checks = linkInfo?.checks || [];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f4ff", padding: "24px 16px" }}>
      <div style={{ background: "#1e2761", borderRadius: "14px", padding: "20px 28px", marginBottom: "24px", display: "flex", alignItems: "center", gap: "16px", maxWidth: "800px", margin: "0 auto 24px" }}>
        <img src="/images/login/logo.png" alt="logo" style={{ width: "100px", height: "36px", objectFit: "contain", borderRadius: "6px" }} />
        <div>
          <h1 style={{ color: "#fff", fontSize: "18px", fontWeight: 700, margin: 0 }}>BGV Verification Portal</h1>
          <p style={{ color: "#cad2e1", fontSize: "13px", margin: 0 }}>
            {linkInfo?.caseId && <>Case: <strong style={{ color: "#02c39a" }}>{linkInfo.caseId}</strong> · </>}
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        <form onSubmit={handleSubmit}>
          {checks.map((ck) => {
            ...
            const docs = checkDetails[ck]?.documents || {};

            return (
              <div key={ck} style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8ecf4", overflow: "hidden", marginBottom: "20px" }}>
                <div style={{ background: "#1e2761", padding: "14px 20px" }}>
                  <h3 style={{ color: "#fff", fontSize: "14px", fontWeight: 700, margin: 0 }}>{config.label}</h3>
                </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "20px" }}>
                      {config.fields.map((f) => (
                        <div key={f.key} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                          ...
                          {f.type === "select" ? (
                            <select value={fieldsState[ck]?.[f.key] || ""} onChange={(e) => setField(ck, f.key, e.target.value)} style={inputStyle}>
                              <option value="">— Select —</option>
                              {f.options.map((o) => <option key={o} value={o}>{o}</option>)}
                            </select>
                          ) : f.type === "textarea" ? (
                            <textarea value={fieldsState[ck]?.[f.key] || ""} onChange={(e) => setField(ck, f.key, e.target.value)} rows={3} style={{ ...inputStyle, resize: "vertical" }} />
                          ) : (
                            <input type={f.type} value={fieldsState[ck]?.[f.key] || ""} onChange={(e) => setField(ck, f.key, e.target.value)} style={inputStyle} />
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  {config.documents.length > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      {config.documents.map((d) => {
                        ...
                        const uploading = uploadingKey === `${ck}:${d.key}`;
                        return (
                          <div key={d.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", borderRadius: "10px", border: uploaded ? "2px solid #02c39a" : "1.5px solid #e2e8f0", background: uploaded ? "#f0fdf4" : "#f8fafc" }}>
                            <div>
                              <div style={{ fontSize: "13px", fontWeight: 600, color: "#1e293b" }}>{d.label}</div>
                              {uploaded && <div style={{ fontSize: "11px", color: "#16a34a" }}>✓ {uploaded.name}</div>}
                            </div>
                            <label style={{ padding: "7px 14px", borderRadius: "8px", fontSize: "12px", fontWeight: 700, cursor: "pointer", background: uploaded ? "#02c39a" : "#1e2761", color: "#fff" }}>
                              {uploading ? "Uploading…" : uploaded ? "Replace" : "Upload"}
                              <input type="file" hidden accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => handleUpload(ck, d.key, e.target.files[0])} />
                            </label>
                          </div>
                        );
                      })}
          ...
          <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8ecf4", overflow: "hidden", marginBottom: "20px" }}>
            <div style={{ background: "#1e2761", padding: "14px 20px" }}>
              <h3 style={{ color: "#fff", fontSize: "14px", fontWeight: 700, margin: 0 }}>General Documents</h3>
            </div>
            <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "10px" }}>
              {COMMON_DOCS.map((d) => {
                const uploaded = checkDetails.general?.documents?.[d.key];
                const uploading = uploadingKey === `general:${d.key}`;
                return (
                  <div key={d.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", borderRadius: "10px", border: uploaded ? "2px solid #02c39a" : "1.5px solid #e2e8f0", background: uploaded ? "#f0fdf4" : "#f8fafc" }}>
                    <div>
                      <div style={{ fontSize: "13px", fontWeight: 600, color: "#1e293b" }}>
                        {d.label} <span style={{ marginLeft: "6px", background: "#fef3c7", color: "#b45309", fontSize: "10px", fontWeight: 700, padding: "2px 6px", borderRadius: "4px" }}>REQUIRED</span>
                      </div>
                      {uploaded && <div style={{ fontSize: "11px", color: "#16a34a" }}>✓ {uploaded.name}</div>}
                    </div>
                    <label style={{ padding: "7px 14px", borderRadius: "8px", fontSize: "12px", fontWeight: 700, cursor: "pointer", background: uploaded ? "#02c39a" : "#1e2761", color: "#fff" }}>
                      {uploading ? "Uploading…" : uploaded ? "Replace" : "Upload"}
                      <input type="file" hidden accept=".jpg,.jpeg,.png,.pdf" onChange={(e) => handleUpload("general", d.key, e.target.files[0])} />
                    </label>
                  </div>
                );
              })}
          ...
          <button type="submit" className="primary-cta" disabled={submitting} style={{ width: "100%", fontSize: "15px", height: "52px" }}>
